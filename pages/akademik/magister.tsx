...
function MagisterList() {
  const { programs, isLoading, error } = useStudyProgramList("S2");
        ...
        {programs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
            ...
            <p className="text-muted-foreground max-w-sm">
              Program Magister sedang dalam tahap persiapan. Pantau terus untuk pembaruan terbaru.
            </p>
...
export default function MagisterPage() {
  return (
    <>
      <Head>
        <title>Program Magister (S2) | {AppSettings.appName}</title>
      </Head>
      <MagisterList />
    </>
  );
...
MagisterPage.layout = (page: React.ReactElement) => <MainLayout>{page}</MainLayout>;
