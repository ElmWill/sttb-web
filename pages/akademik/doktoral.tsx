...
function DoktoralList() {
  const { programs, isLoading, error } = useStudyProgramList("S3");
  ...
  return (
    <>
      <PageHero
        ...
        description="Temukan program studi Doktoral di Sekolah Tinggi Teologi Bandung."
      />
      <PageContainer>
        {programs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
            ...
            <p className="text-muted-foreground max-w-sm">
              Program Doktoral sedang dalam tahap persiapan. Pantau terus untuk pembaruan terbaru.
            </p>
...
export default function DoktoralPage() {
  return (
    <>
      <Head>
        <title>Program Doktoral (S3) | {AppSettings.appName}</title>
      </Head>
      <DoktoralList />
    </>
  );
...
DoktoralPage.layout = (page: React.ReactElement) => <MainLayout>{page}</MainLayout>;
